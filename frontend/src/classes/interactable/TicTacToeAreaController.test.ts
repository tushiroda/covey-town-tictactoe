...
describe('[T1] TicTacToeAreaController', () => {
  ...
  describe('[T1.1]', () => {
    ...
    describe('makeMove', () => {
      it('should throw an error if the game is not in progress', async () => {
        ...
        await expect(() => controller.makeMove(0, 1)).rejects.toThrowError('No game in progress');
      });
      it('Should call townController.sendInteractableCommand', async () => {
        const controller = ticTacToeAreaControllerWithProp({
          ...
          o: ourPlayer.id,
          winner: undefined,
          _id: mockTownController.townID,
        });

        const mockSendInteractableCommand = jest.spyOn(
          mockTownController,
          'sendInteractableCommand',
        );
        mockSendInteractableCommand.mockImplementation(async () => {
          return new Promise(resolve => {
            resolve('called');
          });
        });

        const mockMakeMove = jest.spyOn(controller, 'makeMove');
        mockMakeMove.mockImplementation(async () => {
          await mockTownController.sendInteractableCommand('makeMove', {
            type: 'GameMove',
            gameID: controller.id,
            move: { gamePiece: 'X', row: 0, col: 1 },
          });
        });

        controller.makeMove(0, 1);

        expect(mockSendInteractableCommand).toHaveBeenCalledWith('makeMove', {
          type: 'GameMove',
          gameID: controller.id,
          move: { gamePiece: 'X', row: 0, col: 1 },
        });
  ...
  describe('[T1.2] _updateFrom', () => {
    describe('if the game is in progress', () => {
      ...
      let spy: jest.SpyInstance<boolean, [event: string | number, ...args: string[]]>;
      beforeEach(() => {
        ...
        spy = jest.spyOn(controller, 'emit');
      });
      it('should emit a boardChanged event with the new board', () => {
        ...
        controller.updateFrom(newModel, controller.players);
        expect(spy).toHaveBeenCalledWith('boardChanged', controller.board);
      });
      it('should not emit a boardChanged event if the board has not changed', () => {
        controller.updateFrom(controller.toInteractableAreaModel(), controller.players);
        expect(spy).not.toHaveBeenCalledWith('boardChanged', controller.board);
      });
      it('should emit a turnChanged event with true if it is our turn', () => {
        const model = controller.toInteractableAreaModel();
        let newMoves: ReadonlyArray<TicTacToeMove> = [
          {
            gamePiece: 'X',
            row: 0 as TicTacToeGridPosition,
            col: 0 as TicTacToeGridPosition,
        ...
        assert(model.game);
        let newModel: GameArea<TicTacToeGameState> = {
          ...model,
          game: {
            ...model.game,
            state: {
      ...
      it('should not emit a turnChanged event if the turn has not changed', () => {
        ...
        controller.updateFrom(newModel, controller.players);
        expect(spy).not.toHaveBeenCalledWith('turnChanged', controller.isOurTurn);
      });
